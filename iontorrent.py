...
class TorrentSuiteProjector(Projector):
    ...
    def old_prepare_prototypes(self):
        ...
        experiment_prototype.uri = '[object["resource_uri"] for object in environment["objects"]]'

        exp_metadata_prototype = ProjectionPrototype('file', experiment_prototype)
        exp_metadata_prototype.name = "'metadata.json'"
        exp_metadata_prototype.uri = '[environment["resource_uri"]]'
        ...
        exp_plan_metadata_prototype.uri = '[environment["plan"]]'
        ...
        result_prototype.uri = "[res for res in environment['results']]"

        sample_prototype = ProjectionPrototype('directory', result_prototype)
        sample_prototype.name = "content['name']"
        sample_prototype.uri = "[sample['resource_uri'] for sample in context[1]['samples']]"

        sample_metadata_prototype = ProjectionPrototype('file', experiment_prototype)
        sample_metadata_prototype.name = "'metadata.json'"
        sample_metadata_prototype.uri = '[environment["resource_uri"]]'

        bam_prototype = ProjectionPrototype('file', sample_prototype)
        bam_prototype.name = "environment['name']+'.bam'"
        bam_prototype.uri = "['/auth/output/Home/'+path.split(context[2]['filesystempath'])[1] + '/'" \
                            "+fetch_context(context[1]['plan'])['barcodedSamples'][environment['name']]['barcodes'][0]" \
                            " + '_rawlib.bam']"
        ...
        plugin_result_prototype.uri = "[p_res for p_res in context[2]['pluginresults']" \
                                      " if 'variant' in fetch_context(p_res)['pluginName'] and 'VFNA' not in fetch_context(p_res)['pluginName']]"
        ...
        bed_prototype.uri = "['/auth/output/Home/' + path.basename(context[2]['filesystempath']) + '/plugin_out/'" \
                                " + path.basename(environment['path']) " \
                                " + '/' + path.basename(environment['store']['targets_bed'])]"
        ...
        local_settings_prototype.uri = "['/auth/output/Home/' + path.basename(context[2]['filesystempath']) + '/plugin_out/' + path.basename(environment['path'])  + '/' + 'local_parameters.json' ]"
